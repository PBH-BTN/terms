...
PeerBanHelper 作为一个注重隐私的开源项目，绝不会滥用自己的权限，收集用户额外信息。秉承透明、公开、可控、正当使用原则向您披露我们对收集的信息如何处理、使用。  
目前 PeerBanHelper 获取并传输的所有数据如下：

* 您的匿名安装标识符、PeerBanHelper 的版本号，以及操作系统类型与版本号等基本信息
  * 我们用此数据来确定目前有多少用户在使用我们的程序，及其版本分布情况。同时，我们也用此数据来了解用户所使用的操作系统类型、指令集、与版本分布情况，以便我们在开发过程中进行针对性优化
* 程序发生故障时，收集的错误信息和错误上下文
  * 我们收集、使用这些数据用于分析程序故障原因，并对其实施修复以提升程序稳定性
* 您在浏览 PeerBanHelper WebUI 时，访问不同功能页面的动作、及其在对应页面上停留的时间
  * 我们使用此数据了解 WebUI 上的哪些功能被更多使用，以便改善使用体验

收集的所有数据均为匿名数据，不包含您的个人隐私信息。同时我们承诺不向任何第三方出售、出租、共享收集的任何信息。所有的数据将仅用于 PeerBanHelper 项目的开发与改进，且只能被 PBH-BTN 的受许可的成员访问。

### 从信息收集中退出

若需要退出信息收集，请访问您的 PeerBanHelper 安装的 WebUI，转到“设置->基础设置”，关闭 “启用遥测” 开关，保存并刷新页面，即可退出信息收集。  
退出后，PeerBanHelper 将不再收集您的任何信息（下述例外情况除外）。如需删除已存储的数据，请通过下面的联系方式与我们联系。
